...
class CosineAnnealingLRWarmup(_LRScheduler):
    ...
    def get_warmup_lr(self):
        return [
            ((self.warmup_lr - self.base_lr) / (self.warmup_epochs - 1) * (self.last_epoch - 1) + self.base_lr)
            / self.base_lr
            * base_lr
            for base_lr in self.base_lrs
        ]
...
def init_weights(model, weights_path, caffe=False, classifier=False):
    ...
    if not classifier:
        if caffe:
            weights = {k: weights[k] if k in weights else model.state_dict()[k] for k in model.state_dict()}
        else:
            weights = weights["state_dict_best"]["feat_model"]
            weights = {
                k: weights["module." + k] if "module." + k in weights else model.state_dict()[k]
                for k in model.state_dict()
            }
    else:
        weights = weights["state_dict_best"]["classifier"]
        weights = {
            k: weights["module.fc." + k] if "module.fc." + k in weights else model.state_dict()[k]
            for k in model.state_dict()
        }
