...
class model:
    ...
    def init_models(self, optimizer=True):
        ...
        if self.meta_sample:
            # init meta optimizer
            self.optimizer_meta = torch.optim.Adam(
                self.learner.parameters(), lr=self.training_opt["sampler"].get("lr", 0.01)
            )
    ...
    def init_criterions(self):
        ...
        self.criterion_defs = criterion_defs
        self.criterions = {}
    ...
    def init_optimizers(self, optim_params):
        ...
        if self.config["coslr"]:
            ...
            print(
                "===> Using coslrwarmup eta_min={}, warmup_epochs={}".format(
            ...
            scheduler = optim.lr_scheduler.StepLR(
                optimizer, step_size=self.scheduler_params["step_size"], gamma=self.scheduler_params["gamma"]
            )
    ...
    def batch_loss(self, labels, indexes=None):
        self.loss = 0
        ...
        if "PerformanceLoss" in self.criterions.keys():
            kwargs = {
                "epoch": self.epoch,
                "crit_def": self.criterion_defs["PerformanceLoss"],
                "indexes": indexes,
            }
            self.loss_perf = self.criterions["PerformanceLoss"](self.logits, labels, **kwargs)
    ...
    def meta_forward(self, inputs, labels, verbose=False):
        ...
        with higher.innerloop_ctx(self.networks["classifier"], self.model_optimizer) as (fmodel, diffopt):
            ...
            loss = self.criterions["PerformanceLoss"](train_outputs, labels, reduction="none", **kwargs)
                ...
                with torch.set_grad_enabled(True):
                    ...
                    if step % self.training_opt["display_step"] == 0:
                        ...
                        print_str = [
                            ...
                            "Step: %5d" % (step),
                            "Minibatch_loss_feature: %.3f" % (minibatch_loss_feat) if minibatch_loss_feat else "",
                            "Minibatch_loss_performance: %.3f" % (minibatch_loss_perf) if minibatch_loss_perf else "",
    ...
    def eval_with_preds(self, preds, labels):
        ...
        if len(mixup_preds) > 0:
            mixup_preds, mixup_labels, mixup_ws = list(
                map(np.concatenate, [mixup_preds * 2, mixup_labels1 + mixup_labels2, mixup_ws])
            )
            ...
            (
                ...
                n_top1_median,
                n_top1_low,
            ) = weighted_shot_acc(mixup_preds, mixup_labels, mixup_ws, self.data["train"])
    ...
    def eval(self, phase="val", openset=False, save_feat=False):
        ...
        self.eval_f_measure = F_measure(
            preds, self.total_labels, openset=openset, theta=self.training_opt["open_threshold"]
        )
